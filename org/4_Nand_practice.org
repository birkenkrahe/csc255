...
* Model the gate in LogiSim Evolution
...
* Extract the compare file from the model
...
4) Alternatively, you can write a C++ or Python script to create the
   ...
   - This is how your =Nand.cmp= file (in the same directory) should
     look like:
...
2) Replace the comment at the top by the following:
   ...
   #+begin_src txt :tangle ~/Nand.hdl
     ...
     CHIP Nand {
         ...
         PARTS:
          And(a=a, b=b, out=aAndb ); // template for first gate
          Not(in=aAndb, out=out );   // template for second gate
     }
   #+end_src
...
* Open the HDL simulator desktop software
...
3) The last part of the test script is a series of statements of the
   ...
   For the Nand chip, the =Nand.tst= script looks like this (without the
   C-style comments - though they should not hurt):
   #+begin_src txt :tangle ~/nand2tetris/projects/mb2/Nand.tst
       load Nand.hdl,           // load the CHIP
       output-file Nand.out,    // specify where to write the output
       compare-to Nand.cmp,     // specify what to compare it with
       output-list a b out;     // generate the output file header

       set a 0, set b 0, eval, output;   // check binary values
