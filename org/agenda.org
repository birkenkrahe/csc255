...
3. Why is output from multiple C++ threads using ~std::cout~ potentially
   unsafe without a =mutex= ("mutual exclusion")?
   #+begin_quote
   Because the ~ostream~ buffer is not protected, outputs from different
   threads can become interleaved.
   #+end_quote
...
7. If the stack grows downward and the heap grows upward, why don't
   ...
   #+RESULTS:
   #+begin_example
   global variable address: 0x55914f99c010

   --- Stack growth ---
   stack depth 1 local_var address: 0x7ffd7f8c7414
   stack depth 2 local_var address: 0x7ffd7f8c73e4
   stack depth 3 local_var address: 0x7ffd7f8c73b4
   stack depth 4 local_var address: 0x7ffd7f8c7384
   stack depth 5 local_var address: 0x7ffd7f8c7354

   --- Heap growth ---
   heap allocation 0 address: 0x5591631c8ec0
   heap allocation 1 address: 0x5591631c8ee0
   heap allocation 2 address: 0x5591631c8f00
   heap allocation 3 address: 0x5591631c8f20
   heap allocation 4 address: 0x5591631c8f40
   #+end_example
